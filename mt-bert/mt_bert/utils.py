...
def load_label_keywords(file, output_file, n_class):
    label_dict = {}
    with open(file, 'r', encoding='utf-8') as f:
        reader = csv.reader(f, delimiter='\t')
        for i, blocks in enumerate(reader):
            assert len(blocks) == 2, '%s:%s: number of blocks: %s' % (file, i, len(blocks))
            label_dict[blocks[0]] = blocks[1]
        assert len(label_dict) == n_class
