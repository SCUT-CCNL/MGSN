...
class MTDNNModel(object):
    ...
    def update(self, batch_meta, batch_data):
        ...
        if len(inputs) == 5 and 'premise_mask' in batch_meta:
            inputs.append(None)
            inputs.append(None)
            inputs.append(None)
            ...
            inputs = inputs[0:3] + [None, None] + inputs[3:] + [None, None]

        elif len(inputs) == 5 and 'label_word_id' in batch_meta:
            inputs = inputs[0:3] + [None, None] + inputs[3:]
            inputs.append(batch_data[batch_meta['label_mask']])
        else:
            print('error')
        ...
        if task_type == TaskType.Span:
            ...
            logits, label_logits = self.mnetwork(*inputs)
            # print(logits.size())
            # print(label_logits.size())
            rg_weight = self.config['rg_weight']

            loss = F.cross_entropy(logits, y) + F.cross_entropy(label_logits, y)
        else:
            logits = self.mnetwork(*inputs)
            # print('logits: ', logits)
            if task_type == TaskType.Ranking:
    ...
    def predict(self, batch_meta, batch_data):
        ...
        if len(inputs) == 5 and 'premise_mask' in batch_meta:
            ...
            inputs = inputs[0:3] + [None, None] + inputs[3:] + [None]  # model inference without label_mask
        ...
        if task_type == TaskType.Ranking:
            ...
            seq_score, _ = self.mnetwork(*inputs)
            seq_score = F.softmax(seq_score, dim=1)
            seq_score = seq_score.data.cpu()
            seq_score = seq_score.numpy()
            predict = np.argmax(seq_score, axis=1).tolist()
            seq_score = seq_score.reshape(-1).tolist()
            return seq_score, predict, batch_meta['label']
        else:
            score = self.mnetwork(*inputs)
    ...
    def test(self, batch_meta, batch_data):
        self.network.train()
        labels = batch_data[batch_meta['label']]
        soft_labels = None
        if self.config.get('mkd_opt', 0) > 0 and ('soft_label' in batch_meta):
