...
from module.biaffine_pairwise import BiaffinePairwiseScore, BiaffinePairwiseScore_2, BiaffinePairwiseScore_3, RE
from module.label_structure import SeqMatchLabel3
...
class SANBertNetwork(nn.Module):
    def __init__(self, opt, bert_config=None):
        ...
        # self.biaffine_list = nn.ModuleList()
        # self.seq_match_label = nn.Module()
        self.scoring_list = nn.ModuleList()
        # self.label_scoring_list = nn.ModuleList()
        ...
        for task, lab in enumerate(labels):
            ...
            elif task_type == TaskType.RE:
                out_proj = RE(hidden_size, lab)

            elif task_type == TaskType.RelationExtraction:
                out_proj = BiaffinePairwiseScore_3(hidden_size, lab)
                # out_proj = BiaffinePairwiseScore_2(hidden_size, lab)
                # out_proj = BiaffinePairwiseScore(hidden_size, lab)
            else:
                if decoder_opt == 1:
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, premise_mask=None, hyp_mask=None, ep_dist=None,
                label_word_ids=None, label_mask=None, task_id=0):
        ...
        #     sequence_output = self.dropout_list[task_id](sequence_output)
        #
        #     seq2label, label2seq = self.seq_match_label(label_word_ids, sequence_output, label_mask)
        #
        #     logits, _ = self.scoring_list[task_id](seq2label, ep_dist)
        #     label_logits = self.label_scoring_list[task_id](label2seq)
        #     return logits, label_logits
        elif task_type == TaskType.RelationExtraction:

            sequence_output = self.dropout_list[task_id](sequence_output)
            pooled_output = self.dropout_list[task_id](pooled_output)
            # logits, _ = self.scoring_list[task_id](sequence_output, pooled_output, ep_dist)
            logits, _ = self.scoring_list[task_id](sequence_output, ep_dist)
            return logits
        else:
            if decoder_opt == 1:
                ...
                pooled_output = self.dropout_list[task_id](pooled_output)
                # print(self.scoring_list[0].weight)
                # print(self.scoring_list[1].weight)
                logits = self.scoring_list[task_id](pooled_output)
