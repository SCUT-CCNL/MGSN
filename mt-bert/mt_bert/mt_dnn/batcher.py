...
class BatchGen:
    def __init__(self, data, batch_size=32, gpu=True, is_train=True,
                 maxlen=128, dropout_w=0.005, label_word_num=5, n_class=2, label_mapper=None,
        ...
        self.maxlen = maxlen
        self.label_word_num = label_word_num
        ...
        if do_batch:
            ...
            self.data = BatchGen.make_baches(data, is_train, batch_size)
        self.offset = 0
        self.dropout_w = dropout_w

    @staticmethod
    def make_baches(data, is_train=False, batch_size=32):
        # if is_train:
        #     data_batch = []
    ...
    @staticmethod
    def load(path, is_train=True, maxlen=512, factor=1.0, task_type=None):
    ...
    def __iter__(self):
        while self.offset < len(self):
            ...
            if self.is_train:
                ...
                elif self.task_type in (TaskType.Classification, TaskType.Ranking, TaskType.RelationExtraction, TaskType.RE):
                    batch_data.append(torch.LongTensor(labels))
                    batch_info['label'] = len(batch_data) - 1
                elif self.task_type == TaskType.REWithLabelEmbedding:
                    batch_data.append(torch.LongTensor(labels))
                    batch_info['label'] = len(batch_data) - 1
                    batch_size = self._get_batch_size(batch)
                    tensor_labels = torch.LongTensor(labels).reshape((batch_size, 1))
                    label_mask = torch.FloatTensor(batch_size, self.n_class).fill_(0)
                    batch_data.append(label_mask.scatter_(1, tensor_labels, 1.0))
                    batch_info['label_mask'] = len(batch_data) - 1  # use to label embedding
    ...
    def _prepare_model_input(self, batch):
        ...
        tok_len = self._get_max_len(batch, key='token_id')
        # tok_len = 512
        label_words_num = self.label_word_num
        ...
        if self.task_type == TaskType.RelationExtraction:
            ep_dists = torch.FloatTensor(batch_size, tok_len, tok_len).fill_(-1e8)
        if self.task_type == TaskType.REWithLabelEmbedding:
            ep_dists = torch.FloatTensor(batch_size, tok_len, tok_len).fill_(-1e8)
            label_word_ids = torch.LongTensor(batch_size, label_words_num * self.n_class).fill_(1)
        for i, sample in enumerate(batch):
            ...
            if self.task_type == TaskType.RelationExtraction or self.task_type == TaskType.REWithLabelEmbedding:
                # get location of entity pair
                e1_dist = torch.FloatTensor(sample['e1_dist'] + [0] * (tok_len - len(sample['e1_dist'])))
                e2_dist = torch.FloatTensor(sample['e2_dist'] + [0] * (tok_len - len(sample['e2_dist'])))
                for ei in torch.where(e1_dist == 1)[0]:
                    for ej in torch.where(e2_dist == 1)[0]:
                        ep_dists[i, ei, ej] = torch.FloatTensor([0.0])
                if self.task_type == TaskType.REWithLabelEmbedding:
                    label_names = self.label_mapper.tok2ind.keys()
                    for j, name in enumerate(label_names):
                        assert label_words_num <= len(sample[name]), \
                            'label_words_num is larger than the number of label keywords '
                        label_word_ids[i, j*label_words_num:(j+1)*label_words_num] = torch.LongTensor(sample[name][:label_words_num])
        ...
        if self.__if_pair__(self.data_type):
        ...
        elif self.task_type == TaskType.RelationExtraction:
            batch_info = {
                'token_id': 0,
                'segment_id': 1,
                'mask': 2,
            ...
            batch_data = [token_ids, type_ids, masks, ep_dists]
        elif self.task_type == TaskType.REWithLabelEmbedding:
            batch_info = {
                'token_id': 0,
                'segment_id': 1,
            ...
            batch_data = [token_ids, type_ids, masks, ep_dists, label_word_ids]
        else:
            batch_info = {
                'token_id': 0,
                'segment_id': 1,
