...
class MTDNNModel(object):
    def __init__(self, opt, state_dict=None, num_train_step=-1):
        ...
        if state_dict:
            state_dict.pop('optimizer', 'default')
            cur_state_dict = deepcopy(state_dict)
            pop_state = {k: cur_state_dict['state'].pop(k) for k, v in state_dict['state'].items() if
                         k.startswith('scoring_list')}
            # print(cur_state_dict)
            # print(pop_state)
            if opt['reuse'] and opt['train_datasets'][0] in opt['multi-task_names']:
                # print('##########')
                cur_finetuning_task = opt['train_datasets'][0]
                train_task_idx = opt['multi-task_names'].index(cur_finetuning_task)

                if cur_finetuning_task == 'cdr' or cur_finetuning_task == 'ppim':
                    cur_state_dict['state']['scoring_list.0.weight'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.weight']
                    cur_state_dict['state']['scoring_list.0.conv1.weight'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv1.weight']
                    cur_state_dict['state']['scoring_list.0.conv1.bias'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv1.bias']

                    cur_state_dict['state']['scoring_list.0.conv2.weight'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv2.weight']
                    cur_state_dict['state']['scoring_list.0.conv2.bias'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv2.bias']

                    cur_state_dict['state']['scoring_list.0.conv3.weight'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv3.weight']
                    cur_state_dict['state']['scoring_list.0.conv3.bias'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv3.bias']

                    cur_state_dict['state']['scoring_list.0.conv4.weight'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv4.weight']
                    cur_state_dict['state']['scoring_list.0.conv4.bias'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.conv4.bias']

                    cur_state_dict['state']['scoring_list.0.linear.weight'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.linear.weight']
                    cur_state_dict['state']['scoring_list.0.linear.bias'] = pop_state[
                        'scoring_list.' + str(train_task_idx) + '.linear.bias']

                else:
                    cur_state_dict['state']['scoring_list.0.weight'] = pop_state['scoring_list.' + str(train_task_idx) + '.weight']
                    cur_state_dict['state']['scoring_list.0.bias'] = pop_state['scoring_list.' + str(train_task_idx) + '.bias']
                    # print(cur_state_dict['state']['scoring_list.0.weight'])
                    # print(cur_state_dict['state']['scoring_list.0.bias'])

            else:
                print('not reuse classifer or cur_task is not in multi-task training')
    ...
    def update(self, batch_meta, batch_data):
        ...
        if len(inputs) == 5:
            ...
            inputs = inputs[0:3] + [None, None] + inputs[3:]
        else:
            print('error')
    ...
    def predict(self, batch_meta, batch_data):
        ...
        if task_type == TaskType.Ranking:
            ...
            if task_type == TaskType.Classification or TaskType.RelationExtraction:
                score = F.softmax(score, dim=1)
            score = score.data.cpu()
            score = score.numpy()
