...
from blue_inference import eval_model
from data_utils.log_wrapper import create_logger
from data_utils.task_def import EncoderModelType
from data_utils.utils import set_environment
# from torch.utils.tensorboard import SummaryWriter
from mt_dnn.batcher import BatchGen
from mt_dnn.finetune_model import MTDNNModel
...
def data_config(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument('--task_def', type=str, default="experiments/blue/task_def.yml")
...
def train_config(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    # reuse classifier of mt-task training into task specific finetuing
    parser.add_argument('--reuse', action='store_true', help="reuse classifer of mt-task training")
...
def main():
    ...
    for dataset in args.train_datasets:
        ...
        # use train
        train_path = os.path.join(args.data_dir, f'{dataset}_train_shuffle_2.json')
    ...
    for dataset in args.test_datasets:
        ...
        # dev_path = os.path.join(args.data_dir, f'{dataset}_dev_2.json')
        # dev_data = BatchGen(
        ...
        test_path = os.path.join(args.data_dir, f'{dataset}_test_2.json')
        test_data = BatchGen(
    ...
    if os.path.exists(bert_model_path):
        ...
        opt['num_hidden_layers'] = config['num_hidden_layers']
        opt['num_attention_heads'] = config['num_attention_heads']
        opt['hidden_act'] = config['hidden_act']
        opt['intermediate_size'] = config['intermediate_size']
        opt['hidden_dropout_prob'] = config['hidden_dropout_prob']
        opt['attention_probs_dropout_prob'] = config['attention_probs_dropout_prob']
        opt['max_position_embeddings'] = config['max_position_embeddings']
        opt['type_vocab_size'] = config['type_vocab_size']
        opt['initializer_range'] = config['initializer_range']

        # add multi-task names

        opt['multi-task_names'] = config['train_datasets']
